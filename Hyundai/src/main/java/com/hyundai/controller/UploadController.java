...
public class UploadController {
	...
	public ResponseEntity<List<UploadResultDTO>> uploadFiles(@PathVariable("bno") long bno, MultipartFile[] uploadFiles )  {
		...
		for(MultipartFile file: uploadFiles){  // 파일 여러 개 처리 가능
			...
			String saveName = uploadPath + File.separator + folderPath + File.separator + uuid + "_" + originalName;  // 저장할 파일 이름
			Path savePath = Paths.get(saveName);  // 파일 저장 경로
           
			// 이미지 파일 저장
			try{
				file.transferTo(savePath);
				String thumnailSaveName = uploadPath + File.separator + folderPath + File.separator + "s_" + uuid + "_" + originalName;
